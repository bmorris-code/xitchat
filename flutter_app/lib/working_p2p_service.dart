...
class WorkingP2PService {
  ...
  Future<bool> initialize() async {
    try {
      debugPrint('🔧 Initializing Working P2P Service...');
      ...
      debugPrint('✅ Working P2P Service initialized - ID: $_myPeerId');
      return true;
    } catch (error) {
      debugPrint('❌ Working P2P initialization failed: $error');
      return false;
    }
  }
      ...
      for (BluetoothService service in services) {
        if (service.uuid.toString() == '12345678-1234-1234-1234-123456789abc') {
          ...
          for (BluetoothCharacteristic characteristic in service.characteristics) {
            if (characteristic.uuid.toString() == '12345678-1234-1234-1234-123456789abd') {
  ...
  Future<void> _broadcastWebRTCOffer() async {
    ...
    await prefs.setString('xitchat-webrtc-offer-$_myPeerId', jsonEncode(offerData));
  }
    ...
    for (final key in keys) {
      if (key.startsWith('xitchat-webrtc-offer-') && !key.contains(_myPeerId)) {
        try {
          ...
          if (DateTime.now().millisecondsSinceEpoch - offerData['timestamp'] < 30000) {
  ...
  Future<void> _createWebRTCConnection(String peerId, Map<String, dynamic> offerData) async {
    try {
  ...
  Future<void> _sendWebRTCAnswer(String peerId, RTCSessionDescription answer) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    await prefs.setString('xitchat-webrtc-answer-$_myPeerId-$peerId', jsonEncode(answerData));
  }
  ...
  Future<void> _sendIceCandidate(String peerId, RTCIceCandidate candidate) async {
    ...
    await prefs.setString('xitchat-webrtc-ice-$_myPeerId-$peerId-${DateTime.now().millisecondsSinceEpoch}', jsonEncode(candidateData));
  }
  ...
  String createLocalRoom(String name) {
    final roomId = 'room-${DateTime.now().millisecondsSinceEpoch}-${_myPeerId.substring(5, 9)}';
  ...
  void _inviteNearbyPeers(String roomId) {
    ...
    for (final peer in nearbyPeers) {
      sendMessage(peer.id, jsonEncode({
        'type': 'room_invite',
        'roomId': roomId,
        'roomName': room.name,
      }));
  ...
  List<MeshPeer> getConnectedPeers() => _peers.values.where((p) => p.isConnected).toList();
