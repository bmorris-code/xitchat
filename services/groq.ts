...
export const getXitBotResponseGroq = async (userMessage: string): Promise<string> => {
  ...
  if (now - lastGlobalApiCall < GLOBAL_API_COOLDOWN || now - lastChatApiCall < CHAT_API_COOLDOWN) {
    console.debug('Groq soft cooldown active, continuing for real-time chat');
  }
  ...
  try {
    const response = await groq.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: `You generate quick replies for XitChat.
Replies must be 1-3 words.
Return JSON array of strings.`
        },
        {
          role: 'user',
          content: `Suggest 3 short quick replies for: "${lastMessage}"`
        }
      ],
  ...
  try {
    ...
    const response = await groq.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: `Create 5 buzz items for XitChat as JSON array with:
title, time, snippet, category.`
        },
        {
          role: 'user',
