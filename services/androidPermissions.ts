...
export class AndroidPermissionsService {
  ...
  async requestPushPermissions(): Promise<PermissionResult> {
    ...
    try {
      ...
      const receivePermission = permissions.receive;
      ...
      return {
        granted: receivePermission === 'granted',
        denied: receivePermission === 'denied',
        permanentlyDenied: receivePermission === 'denied',
        canAskAgain: receivePermission !== 'denied'
      };
    } catch (error) {
